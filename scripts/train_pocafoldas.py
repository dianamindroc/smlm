...
def train(config, exp_name=None, fixed_alpha=None):
    ...
    def wandb_set_config(key, value):
        if use_wandb and wandb is not None and getattr(wandb, "config", None) is not None:
    ...
    if use_wandb:
        wandb_run = wandb.init(
            project=os.getenv("WANDB_PROJECT", "smlm"),
            name=exp_name if exp_name is not None else os.path.basename(log_dir.rstrip('/')),
            config=train_config,
            dir=log_dir,
            reinit=True,
        )
    ...
    for epoch in range(1, train_config['num_epochs'] + 1):
        ...
        for i, data in enumerate(train_dataloader):
            ...
            # forward propagation
            if train_config['autoencoder']:
                outputs = model(c_anisotropic)
            else:
                outputs = model(p)

            if train_config['classifier']:
                coarse_pred, dense_pred, _, out_classifier, attn_weights = outputs
            else:
                coarse_pred, dense_pred, _, attn_weights = outputs
                out_classifier = None
            ...
            if train_config['loss'] == 'dcd':
                ...
                wandb_log({'reconstruction_loss_only': loss.item() * 1e3})
                try:
                ...
                wandb_log({'scale_loss': scale_loss.item() * 1e3})
                wandb_log({'total_train_loss': scaled_loss.item() * 1e3})
                wandb_log({'coarse_loss': loss1 * 1e3})
                wandb_log({'dense_loss': loss2 * 1e3})
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(val_dataloader):
                ...
                if train_config['classifier']:
                    coarse_pred, dense_pred, features, out_classifier, attn_weights = outputs
                else:
                    coarse_pred, dense_pred, features, attn_weights = outputs
                    out_classifier = None
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('PocaFoldAS')
