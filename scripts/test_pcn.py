...
def test_single_category(category, model, params, save=True):
    ...
    test_dataset = ShapeNet('/home/dim26fa/data/PCN', 'test_novel' if params.novel else 'test', category)
    ...
    with torch.no_grad():
        for p, c in test_dataloader:
            p, c = p.permute(0, 2, 1), c.permute(0, 2, 1)
...
def test(params, save=False):
    ...
    model = pcn.PCN(16384, 1024, 4).to(params.device)
