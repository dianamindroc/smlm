...
import torchvision
#always have to import torchvision before chamfer_dist, otherwise we get an libcuda error
...
class PCN(nn.Module):
    def __init__(self, num_pred, encoder_channel):
        ...
        a = torch.linspace(-0.05, 0.05, steps=grid_size, dtype=torch.float).view(1, grid_size).expand(grid_size,
                                                                                                      grid_size).reshape(
            1, -1)
        b = torch.linspace(-0.05, 0.05, steps=grid_size, dtype=torch.float).view(grid_size, 1).expand(grid_size,
                                                                                                      grid_size).reshape(
            1, -1)
