...
class PCNDataset(data.Dataset):
    """
    Class for creating a dataset suitable for the PCN Network. Taken from PCN-PyTorch github repository.
    """
    def __init__(self, root_folder, split, category):
        assert split in ['train','valid','test','test_novel'], "split value error!"
    ...
    def _load_data(self):
        with open(os.path.join(self.root_folder, '{}.list').format(self.split)) as f:
        ...
        for line in lines:
            ...
            if self.split == 'train':
                partial_paths.append(os.path.join(self.root_folder, self.split, 'partial', category, model_id + '_{}.ply'))
            else:
